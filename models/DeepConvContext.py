...
class DeepConvContext(nn.Module):
    ...
    """
    def __init__(self, batch_size, channels, classes, window_size, conv_kernels=64, conv_kernel_size=5, lstm_units=128, lstm_layers=2, dropout=0.5, bidirectional=False, type='lstm', attention_num_heads=4, transformer_depth=6):
        ...
        if type == 'lstm':
            ...
            self.is_causal = not bidirectional
            self.context_lstm = torch.nn.Transformer(d_model=lstm_units, nhead=attention_num_heads, num_encoder_layers=transformer_depth, num_decoder_layers=transformer_depth, dim_feedforward=attention_num_heads*lstm_units, batch_first=False)
    ...
    def forward(self, x):
        ...
        if isinstance(self.context_lstm, torch.nn.Transformer):
            ...
            if self.is_causal:
                ...
                attn_mask = None
            x = self.context_lstm(x, x, src_is_causal=self.is_causal, tgt_is_causal=self.is_causal, src_mask=attn_mask, tgt_mask=attn_mask)
